...
async function stop() {
  ...
  if (audio.pendingStart || audio.recording) {
    try {
      await audio.stopRecording();
    } catch (e) {
      console.warn('stopRecording failed during stop', e);
    }
  }
  ...
  if (audio.ctx && audio.ctx.state === 'running') {
    ...
    audio.gain.gain.setTargetAtTime(0.0, t, 0.03);
  }

  state.x = 0.5;
  state.y = 0.5;
  els.dot.style.transform = 'translate(-50%,-50%)';
  els.modeState.textContent = 'Camera: off';
  els.calBtn.disabled = true;
  els.recordBtn.disabled = true;
  els.startBtn.textContent = 'START';
  els.msg.textContent = 'Stopped.';
}
...
function renderLoop() {
  ...
  if (els.video.currentTime !== state.lastVideoTime) {
    ...
    if (lms) {
      ...
      const yMapped = clamp(0.5 + rawDy * 6.2, 0, 1);
...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.register('./sw.js?v=20260313yonly1').then(async reg => {
